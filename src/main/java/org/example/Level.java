...
public class Level {
    ...
    private List<String> bonusSymbols = Arrays.asList("Cherries","Strawberry","Peach","Peach",
            "Apple","Apple","Grapes","Grapes","Galaxian","Galaxian","Bell","Bell"); // Acima é 'Key'

    private List<Integer> bonusPoints = Arrays.asList(1,3,5,5,7,7,10,10,20,20,30,30); // Acima é 50
    ...
    public Level(int ln,int width,int height,TextGraphics graphics) throws IOException {
        ...
        Integer pacManSpeed;
        Integer ghostSpeed;
        Integer pacManFrightSpeed;
        Integer ghostFrightSpeed;
        Integer timeInFright;
        if (levelNumber == 1){
            pacManSpeed = 80;
            pacManFrightSpeed = 90;
            ghostSpeed = 75;
            ghostFrightSpeed = 50;
            timeInFright = 6;
        ...
        if ((levelNumber >= 2 && levelNumber <= 4)){
            ghostSpeed = 85;
            pacManFrightSpeed = 95;
            ghostFrightSpeed = 55;
            timeInFright = 4;
        }else{
            ghostSpeed = 95;
            pacManFrightSpeed = 100;
            ghostFrightSpeed = 60;
            timeInFright = 2;
        }
        if ((levelNumber >= 2 && levelNumber <= 4) || levelNumber == 21)pacManSpeed = 90;
        else pacManSpeed = 100;
