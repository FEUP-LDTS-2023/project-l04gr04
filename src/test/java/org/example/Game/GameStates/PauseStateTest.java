...
public class PauseStateTest {
    ...
    @Test
    void testInput2() throws IOException, UnsupportedAudioFileException, LineUnavailableException {
        Game mockGame = mock(Game.class);
        pauseState pauseState = new pauseState(mockGame);
        KeyStroke arrowDownKey = new KeyStroke(KeyType.ArrowDown);
        KeyStroke enterKey = new KeyStroke(KeyType.Enter);
        pauseState.input(arrowDownKey);
        pauseState.input(enterKey);
        verify(mockGame).warnMapStopMusic();
        verify(mockGame).resetStructs();
